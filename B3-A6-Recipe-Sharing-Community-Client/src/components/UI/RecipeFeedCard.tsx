...
const RecipeFeedCard = ({ recipe }: { recipe: IRecipe }) => {
  const { _id, title, images, user } = recipe || {};
        ...
        <div className="mt-4 flex gap-5">
          {/* {email !== loggedInUser?.email && (
            <>
              ...
              {!loggedInUser?.email && (
                <AuthenticationModal id={_id as string} />
              )}
            </>
          )} */}
          {email !== loggedInUser?.email && (
          ...
          <Link href={`/recipe-details/${_id}`}>Details</Link>
